class CustomFailure < Devise::FailureApp
  def redirect_url
    if warden_options[:scope] == :user && params[:cross_domain_login].present?
      ...
      if (params[:user].present? || params[:merchants_manager].present?) &&
          (instance = (User.find_by_email(params[:user][:email]) || Manager.find_by_email(params[:merchants_manager][:email]) rescue nil)).present?  &&
          instance.authentications.present?
