  ...
  def email_body_shop_logo(shop)
    if shop.has_logo?
      if shop.website.present?
        link_to(image_tag(shop.logo_location, alt: shop.name, title: @shop.name, style:'max-height: 150px; max-width: 100%;').html_safe, shop.website, target: "_blank")
      else
        image_tag(shop.logo_location, alt: shop.name, title: shop.name, style:'max-height: 150px; max-width: 100%;')
      end
    else
      if shop.website.present?
