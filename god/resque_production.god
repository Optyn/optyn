...
0.upto(num_workers) do |num|
  God.watch do |w|
    ...
    w.env      = {"QUEUES"=>"*", "RAILS_ENV"=>rails_env,'PIDFILE' => "#{rails_shared_root}/pids/resque/#{w.name}.pid"}
    w.start    = "QUEUES=* bundle exec rake -f #{rails_current_root}/Rakefile environment resque:work"
    w.log      = "#{rails_shared_root}/log/resque-#{num}.log"
