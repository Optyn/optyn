class ShopCredit < ActiveRecord::Base
  ...
  scope :currently_active, ->(begin_stamp, end_stamp) {where(['shop_credits.begins <= :begin_stamp AND shop_credits.ends >= :end_stamp', 
    {begin_stamp: begin_stamp, end_stamp: end_stamp}])}
